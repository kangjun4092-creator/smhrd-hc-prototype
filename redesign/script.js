...
// 섹션별 주석으로 표시해 두었습니다.
// 실시간 판정은 종목별로 하나씩 붙여나가는 중이다. 스쿼트에 이어 런지·사이드 런지도
// 실제 MediaPipe 무릎 각도 판정을 지원한다 — 기준값은 EXERCISE_REFERENCE 참고.
// 다른 종목 추가 시 여기에 항목을 넣으면 됨(id는 tools/extract-exercise-reference.html의
// EXERCISE_DEFS 키와 맞춰야 함): pushup, plank, burpee
const EXS = [
  {id:'squat', name:'스쿼트', target:'하체 · 둔근', level:'초급'},
  {id:'lunge', name:'런지', target:'하체 · 둔근·허벅지', level:'초급'},
  {id:'sidelunge', name:'사이드 런지', target:'하체 · 내전근·둔근', level:'중급'},
];
// 종목별 정자세 레퍼런스 gif (튜토리얼 화면에 표시). 파일은 redesign/ 폴더 바로 아래에 둔다.
const EX_REFERENCE_GIF = {
  squat:'Bodyweight_Squats.gif',
  lunge:'런지.gif',
  sidelunge:'사이드런지.gif',
};

// 실시간 판정이 붙은 종목(스쿼트/런지/사이드 런지)이 전부 같은 지표를 공유하는 미션 풀이다.
// metric은 saveExerciseResult()에서 세션이 끝날 때마다 누적되는 공용 카운터
// (state.missions.counters)를 가리키고, 일간/주간/월간 미션이 전부 이 같은 카운터를 공유해서
// 어느 종목이든 한 세션으로 세 기간 미션이 동시에 올라간다 (기간별로 카운터가 따로 리셋되는
// 실제 날짜 배치는 없는 프론트 목업이라, "겹쳐서 카운트"되는 쪽이 오히려 지금 요구사항과 맞다).
const MISSION_TEMPLATES = [
  {metric:'reps', label:t=>`하체 운동 ${t}회 달성`, ranges:{daily:[15,30], weekly:[60,140], monthly:[200,450]}, rewardPerUnit:2.5},
  {metric:'perfect', label:t=>`퍼펙트 ${t}개 만들기`, ranges:{daily:[3,8], weekly:[15,40], monthly:[60,150]}, rewardPerUnit:4},
  {metric:'sessions', label:t=>`운동 세트 ${t}회 완료`, ranges:{daily:[1,2], weekly:[3,6], monthly:[10,20]}, rewardPerUnit:35},
...
function generateMissions(period, count){
  const list=[];
...
const state = {
  ...
  missions: {
    period:'daily',
    list: {
      daily: generateMissions('daily',3),
      weekly: generateMissions('weekly',7),
      monthly: generateMissions('monthly',15),
    },
    // 일간/주간/월간 미션이 공유하는 누적 카운터. saveExerciseResult()에서 실시간 판정 지원
    // 종목(스쿼트/런지/사이드 런지) 세션이 저장될 때마다 갱신된다.
...
// 종목별 실시간 판정 기준. tools/extract-exercise-reference.html(종목별 캡처 이미지로
// 관절 각도를 뽑는 공용 도구)로 분석해서 나온 값으로 교체한다 — 아래는 전부 일반적인
// 동작 각도로 잡은 임시값이다(스쿼트도 마찬가지). 도구의 EXERCISE_DEFS에 이미 squat/lunge가
// 등록돼 있고, 사이드 런지는 같은 골반-무릎-발목 삼각형을 재사용하도록 redesign/tools/
// extract-exercise-reference.html에 추가해뒀다 — 실측 캡처 이미지를 넣으면 값을 다듬을 수 있다.
...
// 런지: 앞다리만 깊게 굽혀지는 편측 동작이라 뒷다리/편 다리는 기준에서 제외하고, 더 굽혀진
// 쪽(앞다리) 각도만 본다 — EXERCISE_ANGLE_MODE의 'min'이 이 역할을 한다.
const LUNGE_REFERENCE = {
  standingKneeAngle: 172, bottomKneeAngle: 90,
  perfectTol: 8, greatTol: 15, goodTol: 24,
};
// 사이드 런지: 옆으로 굽히는 다리 하나만 보는 것은 런지와 같지만, 골반 가동범위상 앞런지만큼
// 깊이 앉지 못하는 경우가 많아 목표 각도를 완만하게, 허용오차도 조금 더 넉넉하게 잡았다.
const SIDE_LUNGE_REFERENCE = {
  standingKneeAngle: 172, bottomKneeAngle: 100,
  perfectTol: 8, greatTol: 16, goodTol: 26,
};
// 종목 id → 판정 기준. 값이 있는 종목만 실제 MediaPipe 판정이 붙고, 없는 종목은 자동으로
// 모의 스켈레톤 애니메이션으로 대체된다(startPoseFeedback 참고).
const EXERCISE_REFERENCE = { squat:SQUAT_REFERENCE, lunge:LUNGE_REFERENCE, sidelunge:SIDE_LUNGE_REFERENCE };
// 'avg' = 좌우 무릎 각도를 평균(스쿼트처럼 양다리가 대칭으로 굽혀지는 동작),
// 'min' = 좌우 중 더 굽혀진 쪽만 사용(런지류처럼 한쪽 다리만 굽혀지는 동작).
const EXERCISE_ANGLE_MODE = { squat:'avg', lunge:'min', sidelunge:'min' };
// 실시간 판정이 붙은 종목의 한글 이름 집합 — saveExerciseResult()에서 미션 카운터를
// 갱신할지 판단하는 데 쓴다.
const LIVE_JUDGE_EX_NAMES = new Set(Object.keys(EXERCISE_REFERENCE).map(id=>EXS.find(e=>e.id===id).name));
const EXERCISE_REP_TARGET = 10; // 실시간 판정이 지원되는 운동의 세션당 반복 횟수 제한
...
// 종목별 튜토리얼 안내 문구. 없는 종목은 스쿼트 문구를 기본값으로 쓴다.
const EX_TUTORIAL_STEPS = {
  squat: [
    '발을 어깨너비로 벌리고 무게중심을 뒤꿈치에 둡니다.',
    '허리를 곧게 편 상태로 천천히 내려갑니다.',
    '무릎이 발끝을 넘지 않도록 각도를 유지합니다.',
    '동작 최저점에서 1초 정지 후 천천히 복귀합니다.',
  ],
  lunge: [
    '양발을 어깨너비로 벌리고 허리를 곧게 세워 섭니다.',
    '한쪽 발을 앞으로 크게 내딛으며 무게중심을 낮춥니다.',
    '앞무릎이 발끝을 넘지 않도록 90도 가까이 굽힙니다.',
    '앞발로 바닥을 밀어내며 처음 자세로 복귀합니다.',
  ],
  sidelunge: [
    '양발을 모으고 허리를 곧게 세워 섭니다.',
    '한쪽 다리를 옆으로 크게 벌리며 무게중심을 낮춥니다.',
    '굽힌 무릎이 발끝 방향을 향하도록 유지합니다.',
    '굽힌 다리로 바닥을 밀어내며 처음 자세로 복귀합니다.',
  ],
};
function renderExStepTutorial(){
  const ex=EXS.find(e=>e.id===state.exercise.picked) || EXS[0];
  const hasLiveJudgment = !!EXERCISE_REFERENCE[ex.id];
  const refGif = EX_REFERENCE_GIF[ex.id];
  const steps = EX_TUTORIAL_STEPS[ex.id] || EX_TUTORIAL_STEPS.squat;
  return `
  <div class="grid grid-2">
    <div class="card">
      ...
      <div class="cam-stage" style="aspect-ratio:1/1;margin-bottom:14px;">
        <img src="${refGif}" alt="${ex.name} 정자세 레퍼런스" style="width:100%;height:100%;object-fit:cover;">
      </div>` : ''}
      <ul class="steplist">
        ${steps.map((s,i)=>`<li><span class="num">${i+1}</span>${s}</li>`).join('')}
      </ul>
    </div>
    ${hasLiveJudgment ? renderTutorialMissionList() : ''}
  </div>
...
function renderExStepCam(){
  const hasLiveJudgment = !!EXERCISE_REFERENCE[state.exercise.picked];
  return `
  <div class="grid cal-grid">
    <div>
    ...
    <div class="card">
      ...
      <div class="stat-row" style="margin:0;">
        <div class="stat-box"><div class="num mono" id="live-reps">0${hasLiveJudgment?` / ${EXERCISE_REP_TARGET}`:''}</div><div class="lbl">인식 횟수</div></div>
...
// EXERCISE_REFERENCE에 판정 기준이 있는 종목(스쿼트/런지/사이드 런지)은 실제 MediaPipe
// 판정 루프로, 나머지 운동(아직 기준 데이터 없음)은 기존 모의 스켈레톤 애니메이션으로 분기한다.
function startPoseFeedback(){
  if(EXERCISE_REFERENCE[state.exercise.picked] && state.exercise.camStream) exStartPoseLoop();
...
// mode:'avg'(기본) = 좌우 무릎 각도 평균(스쿼트처럼 양다리가 대칭으로 굽혀지는 동작).
// mode:'min' = 좌우 중 더 굽혀진(각도가 작은) 쪽만 사용 — 런지류처럼 한쪽 다리만 깊게
// 굽혀지는 편측 동작에서, 평균을 내면 굽히지 않은 다리 때문에 값이 왜곡되는 걸 막는다.
function exKneeAngle(landmarks, mode){
  ...
  if(l==null) return r;
  if(r==null) return l;
  return mode==='min' ? Math.min(l,r) : (l+r)/2;
}
...
function exGradeRep(bottomAngle, torsoDrop){
  ...
  const ref=EXERCISE_REFERENCE[state.exercise.picked] || SQUAT_REFERENCE;
  ...
  function loop(){
    ...
    if(landmarks){
      ...
      if(state.exercise.camPhase==='recording'){
        const exRef=EXERCISE_REFERENCE[state.exercise.picked] || SQUAT_REFERENCE;
        const angle=exKneeAngle(landmarks, EXERCISE_ANGLE_MODE[state.exercise.picked]);
        const torsoAngle=exTorsoAngle(landmarks);
        if(angle!=null){
          const standing=exRef.standingKneeAngle;
          if(exRepPhase==='up'){
...
function toggleRecording(){
  ...
  if(state.exercise.camPhase==='idle'){
    if(hasLiveJudgment) startAlignmentGuide();
    else beginRecording();
    return;
  }
...
function beginRecording(){
  ...
  if(hasLiveJudgment && state.exercise.camStream && window.MediaRecorder){
    exRecordedChunks=[];
    try{
  ...
  state.exercise.timerId=setInterval(()=>{
    ...
    if(!hasLiveJudgment && state.exercise.seconds%3===0){
      reps++;
...
function stopRecording(){
  ...
  if(hasLiveJudgment && exMediaRecorder && exMediaRecorder.state!=='inactive'){
...
function generateResult(){
  if(EXERCISE_REFERENCE[state.exercise.picked] && state.exercise.liveReps.length>0){
    ...
    const ex=EXS.find(e=>e.id===state.exercise.picked);
...
function saveExerciseResult(){
  ...
  // 일간/주간/월간 미션이 공유하는 누적 카운터 갱신 (실시간 판정 지원 종목이면 전부 반영)
  if(LIVE_JUDGE_EX_NAMES.has(r.ex)){
...
function renderMission(){
  return `
  <div class="view-head"><h1>미션</h1><p>일간/주간/월간 미션 선택 → 운동으로 달성하고 포인트를 모아보세요</p></div>
  ${renderMissionPick()}`;
}
